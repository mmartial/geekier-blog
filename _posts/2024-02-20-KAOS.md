...
# KAOS (Kolla Ansible OpenStack) Installation
...
* 3. [Pre-installation steps](#Pre-installationsteps)
	* 3.1. [Hardware Enablement (HWE, optional)](#HardwareEnablementHWEoptional)
	* 3.2. [Docker installation](#Dockerinstallation)
	* 3.3. [Passwordless sudo](#Passwordlesssudo)
	* 3.4. [NFS for Cinder](#NFSforCinder)
* 4. [KAOS](#KAOS)
	* 4.1. [Preparation](#Preparation)
	* 4.2. [Deployment](#Deployment)
	* 4.3. [CLI](#CLI)
		* 4.3.1. [OpenStack command line](#OpenStackcommandline)
		* 4.3.2. [OpenStack configuration file](#OpenStackconfigurationfile)
		* 4.3.3. [Cloud Init: Run once](#CloudInit:Runonce)
	* 4.4. [Reconfigure](#Reconfigure)
* 5. [Post-Installation](#Post-Installation)
	* 5.1. [New admin user (UI)](#NewadminuserUI)
		* 5.1.1. [New User + Project: ssh + security groups + quotas (CLI)](#NewUserProject:sshsecuritygroupsquotasCLI)
	* 5.2. [Add an Ubuntu image to Glance](#AddanUbuntuimagetoGlance)
	* 5.3. [Network and Router setup](#NetworkandRoutersetup)
		* 5.3.1. [network](#network)
		* 5.3.2. [router](#router)
	* 5.4. [Starting and accessible our first VM](#StartingandaccessibleourfirstVM)
		* 5.4.1. [Launch Instance](#LaunchInstance)
		* 5.4.2. [Floating IPs](#FloatingIPs)
		* 5.4.3. [Securely accessing Horizon using a reverse proxy](#SecurelyaccessingHorizonusingareverseproxy)
* 6. [Troubleshooting](#Troubleshooting)
	* 6.1. [Broken after a Reboot](#BrokenafteraReboot)
* 7. [Revision History](#RevisionHistory)
...
- A Linux host, here, Ubuntu 22.04 server.
    ...
    - With a `kaos` user for our OpenStack Kolla Ansible installation.
- Networking with routing capabilities (i.e., a home router connecting to the internet). For our private network:
    ...
    - We reserved one unused IP for the OpenStack KeepAlive connection; here `10.0.0.254`.

##  3. <a name='Pre-installationsteps'></a>Pre-installation steps

###  3.1. <a name='HardwareEnablementHWEoptional'></a>Hardware Enablement (HWE, optional)
...
###  3.2. <a name='Dockerinstallation'></a>Docker installation
...
###  3.3. <a name='Passwordlesssudo'></a>Passwordless sudo
...
# Add and adapt kaos as needed
kaos ALL=(ALL) NOPASSWD:ALL

# save the file and test in a new terminal or login
sudo echo works
```

###  3.4. <a name='NFSforCinder'></a>NFS for Cinder
...
###  4.1. <a name='Preparation'></a>Preparation
...
sudo apt install build-essential libpython3-dev libdbus-1-dev cmake libglib2.0-dev
...
###  4.2. <a name='Deployment'></a>Deployment
...
```bash
PLAY RECAP ***************************************************************************************************************************************************************************************************************************************************
localhost                  : ok=348  changed=229  unreachable=0    failed=0    skipped=258  rescued=0    ignored=1
```
...
###  4.3. <a name='CLI'></a>CLI

(still using the `venv`)

####  4.3.1. <a name='OpenStackcommandline'></a>OpenStack command line
...
####  4.3.2. <a name='OpenStackconfigurationfile'></a>OpenStack configuration file

`kolla-ansible post-deploy` will create a `cloud.yml` file that can be added to your default config: `cp /etc/kolla/clouds.yaml ~/.config/openstack` 

####  4.3.3. <a name='CloudInit:Runonce'></a>Cloud Init: Run once
...
###  4.4. <a name='Reconfigure'></a>Reconfigure

If you modify a `globals.yml` configuration option, from `/data/kaos` run `kolla-ansible -i ./all-in-one reconfigure`.

More `kolla-ansible` CLI options at [https://docs.openstack.org/kolla-ansible/ussuri/user/operating-kolla.html](https://docs.openstack.org/kolla-ansible/ussuri/user/operating-kolla.html).

##  5. <a name='Post-Installation'></a>Post-Installation
...
###  5.1. <a name='NewadminuserUI'></a>New admin user (UI)

Login to your OpenStack instance by going to the IP (it is running on port 80) at [http://10.0.0.17](http://10.0.0.17/) 

The `admin` user’s password can be obtained using `fgrep keystone_admin_password /etc/kolla/passwords.yml` 

Using `Project -> Instance -> Overview` gives you a list of used and available resources.
...
####  5.1.1. <a name='NewUserProject:sshsecuritygroupsquotasCLI'></a>New User + Project: ssh + security groups + quotas (CLI)
...
###  5.2. <a name='AddanUbuntuimagetoGlance'></a>Add an Ubuntu image to Glance
...
In `/data/cloudimg` (create if needed), `wget [https://cloud-images.ubuntu.com/jammy/current/jammy-server-cloudimg-amd64.img](https://cloud-images.ubuntu.com/jammy/current/jammy-server-cloudimg-amd64.img)`

We recommend naming with the image date so you know which base image you use, for example, `mv jammy-server-cloudimg-amd64.img ubuntu2204-20240218.img` 

Then, use the `openstack` command line to add the image to the list of available images for all users of your cloud OS:

```bash
openstack image create --disk-format qcow2 --container-format bare --public --property os_type=linux --file ubuntu2204-20240218.img ubuntu2204server-20240218
```
...
###  5.3. <a name='NetworkandRoutersetup'></a>Network and Router setup
...
####  5.3.1. <a name='network'></a>network

Select `Create Network`

- `Network` tab:
    - Give it a name. I recommend “project-net” replacing “project” with your project name.
...
- `Subnet` tab:
    ...
    - For `Network Address`, use a private IP range not currently in use in your network, for example, `10.56.78.0/24`. Each new subnet created will need to be independent.
    - Select `IPv4`.
...
You now have a router connected to the `external net`work, but it is yet to be connected to the “project network.” Hover over to the “router” and select `Add Interface`

- select the “project-subnet” and leave the `IP Address` unspecified; it will use the configured gateway.

When you return to the `Network Topology` page, you will see a valid connected network.

###  5.4. <a name='StartingandaccessibleourfirstVM'></a>Starting and accessible our first VM

####  5.4.1. <a name='LaunchInstance'></a>Launch Instance
...
- The `Volume Size` does not matter if it is less than the flavor’s disk size; the larger one of the two will be selected.
...
If you look into `/data/nfs` you will see the file(s) for your existing disk volumes.

From your instance row’s `Actions` submenu (right), you can `View Logs` or view the `Console` (you can not log in from the displayed terminal; the `ubuntu` user does not have a known password set) 

####  5.4.2. <a name='FloatingIPs'></a>Floating IPs

With our instance `Running`, its `IP Address` is within our subnet range.
...
- None are likely available, so click the `+` and `Allocate IP` from our `external-net` pool.
...
From your user directory, we can `ssh` into the host’s created floating IP using the authorized ssh key and the default cloud image user of `ubuntu`.

For example:

```bash
# Adapt the 10. IP to match your floating IP
ssh -i ~/.ssh/id_ecdsa ubuntu@10.0.0.82
[...]
...
####  5.4.3. <a name='SecurelyaccessingHorizonusingareverseproxy'></a>Securely accessing Horizon using a reverse proxy

If you have a reverse proxy setup on another host and want to benefit from `https` on the dashboard:

- In your reverse proxy, configure the `Proxy Host` as you would normally; here, we will use `os.example.com`
- Edit the `/etc/kolla/horizon/_9999-custom-settings.py` file and add to it:
...
- Restart `horizon` using `docker kill horizon`. Wait a few seconds, and your access via `https://os.example.com` should not present you with a `csrf_failure=Origin checking failed - https%3A//os.example.com does not match any trusted origin error` (in the address bar).
...
##  7. <a name='RevisionHistory'></a>Revision History
20240220-1: Intitial release