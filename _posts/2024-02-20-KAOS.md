...
Revision: 20240416-0 (`init`: 20240220)

- [1. Preamble](#1-preamble)
  - [1.1. How to use this guide](#11-how-to-use-this-guide)
- [2. Requirements](#2-requirements)
- [3. Pre-installation steps](#3-pre-installation-steps)
  - [3.1. Passwordless sudo](#31-passwordless-sudo)
  - [3.2. Hardware Enablement (HWE, optional)](#32-hardware-enablement-hwe-optional)
...
- [4. KAOS](#4-kaos)
  ...
  - [4.2. Deployment](#42-deployment)
    - [4.2.1. Securely accessing Horizon using a reverse proxy (optional)](#421-securely-accessing-horizon-using-a-reverse-proxy-optional)
  - [4.3. CLI](#43-cli)
...
- [6. Troubleshooting](#6-troubleshooting)
  ...
  - [6.4. Delete the installation](#64-delete-the-installation)
...
- **`10.0.0.0/24`** is the `external-net` CIDR.
- **`10.56.78.0/24`** private IP CIDR for the project's subnet.
- **`10.56.78.1`** project's subnet Gateway IP (should be the first IP of the private CIDR set above)
- **`10.56.78.100,10.56.78.200`** DHCP IP range for the private CIDR
- **`os.example.com`** the URL for OpenStack using our HTTPS upgrading reverse proxy. 
...
- A Linux host, here, Ubuntu 22.04 server.
    ...
    - With a `kaosu` user, for our OpenStack Kolla Ansible installation.
      - Create the user and its home, give it a bash shell: `sudo useradd -m -s /bin/bash kaosu`
      - Make the user part of the sudo group: `sudo adduser kaosu sudo`
      - Set a password for the user: `sudo passwd kaosu`
      - Copy your `authorized_keys` file to the `kaosu` user: `rsync -avPR ~/./.ssh/authorized_keys kaosu@10.0.0.17:.`
...
## 3.2. Hardware Enablement (HWE, optional)
...
sudo apt-get install -y libpython3-dev libdbus-1-dev cmake libglib2.0-dev
...
Before we try the deployment, let’s ensure the Python interpreter is the `venv` one. At the top of the `/etc/kolla/all-in-one` file, add `localhost ansible_python_interpreter=/data/kaos/venv/bin/python`.
...
localhost                  : ok=354  changed=242  unreachable=0    failed=0    skipped=259  rescued=0    ignored=1
```
...
### 4.2.1. Securely accessing Horizon using a reverse proxy (optional)
...
``` 
will create a `/etc/kolla/cloud.yml` file that can be added to your default config: 
```bash
mkdir -p ~/.config
...
We recommend naming based on the image date so you know which base image you use, for example, `mv jammy-server-cloudimg-amd64.img ubuntu2204server-20240416.img` 

Then, use the `openstack` command line to add the image to the list of available images for all users of your cloud OS:

```bash
openstack image create --disk-format qcow2 --container-format bare --public --property os_type=linux --file ubuntu2204server-20240416.img ubuntu2204server-20240416
```
...
sudo /data/kaos/venv/share/kolla-ansible/tools/cleanup-host
...
- 20240416-0: Reorganized section + added additional content for customizations + tested with latest version
